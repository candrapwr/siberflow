...
- **File dan shell tools** — `read_file`, `write_file`, `edit_file`, `copy_file`, `list_dir`, `delete_file`, `grep`, `exec`
...
- **Per-tool toggle** — aktif/nonaktifkan tool individual via settings/env (`SIBERFLOW_TOOLS`). Default hanya file ops aktif; `exec`/`db_query`/`ssh`/`excel`/`docx`/`pdf`/`run_browser` opt-in untuk prompt ringan + blast-radius security kecil
...
- **Context optimization** — buang/ringkas tool history dari turn lama (default aktif); current task tetap utuh. Empat mode via `SIBERFLOW_CONTEXT_OPTIMIZE_MODE`: `compact` (default; LLM narrative summary, threshold-triggered, paling kaya konteks), `recent` (seperti summary, tapi sisakan 1 turn terakhir sebelum current turn tetap utuh), `drop` (buang total), atau `summary` (sisakan tag `[SUMMARY]` berisi *signature* per tool). Defense-in-depth: provider & serialization selalu menjamin assistant message punya content atau tool_calls (fix error 400 DeepSeek)
- **Context usage bar** — progress bar live (Desktop + VS Code) yang nunjukkin seberapa penuh context window + di mana auto-compact bakal trigger
- **Parallel tool grouping** — tool calls paralel dalam 1 assistant turn di-grup jadi 1 collapsible card (Desktop + VS Code)
- **Agent General & Agent Explorer tools** — delegasikan task ke helper agent terisolasi yang return summary, jadi context percakapan utama tetap ramping. `agent_general` terima task + daftar tool; `agent_explorer` adalah searcher read-only untuk riset codebase. Aktif default di Desktop & VS Code; CLI/Telegram aktifkan via `SIBERFLOW_SUBAGENT=true`
- **Pre-truncation** — output `read_file`, `exec`, `write_file` dipotong di sumber supaya 1 result gede gak banjiri context. Default ON (`SIBERFLOW_PRE_TRUNCATE`)
- **Skills** (Telegram) — pustaka instruksi reusable `.md` yang dibaca AI on-demand via `read_file` saat sebuah tugas cocok, tanpa menambah tool baru. Library global di `~/.siberflow/telegram-skills/`, aktif untuk semua chat. Manage **admin-only** lewat panel web admin (port 7070, login OTP via `/login`). Format file: frontmatter `name`/`description`/`enabled` + body markdown; AI lihat index ringkas di system prompt, baca body saat dibutuhkan. Skill bersifat read-only bagi AI (write tool tetap sandbox ke workdir)
...
Aktif/nonaktifkan tool individual supaya tool yang gak dipakai gak membebani prompt (~200-300 token per disabled tool) + blast-radius security lebih ketat. Default: **hanya file ops** aktif (`read_file`, `write_file`, `edit_file`, `copy_file`, `list_dir`, `delete_file`, `grep`). `exec`/`db_query`/`ssh_exec`/`sftp`/`excel_script`/`docx_script`/`pdf_script`/`run_browser`/`analyze_image`/`music_generate` default OFF — opt-in. Pengecualian: `task_update` dan `ask_user` selalu on (core UX, tidak muncul di toggle).
...
| Interface | Cara set |
|---|---|
| **CLI** | env `SIBERFLOW_TOOLS=read_file,write_file,edit_file,copy_file,list_dir,delete_file,grep,run_browser` (comma-separated) |
