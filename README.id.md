...
- **Subagent & Explore tools** — delegasikan task ke helper agent terisolasi yang return summary, jadi context percakapan utama tetap ramping. `subagent` terima task + daftar tool; `explore` adalah searcher read-only untuk riset codebase. Aktif default di Desktop & VS Code; CLI/Telegram aktifkan via `SIBERFLOW_SUBAGENT=true`
