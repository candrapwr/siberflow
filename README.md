...
SIBERFLOW_PROVIDER_APP_NAME=siberflow
SIBERFLOW_PROVIDER_HEADERS='{"X-Gateway-Route":"coding"}'
```
...
- **Agent General & Agent Explorer tools** - delegate a task to a focused helper agent that runs in an isolated context and returns a summary, keeping the main conversation lean. `agent_general` takes a task + tool list; `agent_explorer` is a read-only searcher for codebase research. Enabled by default on Desktop and VS Code; CLI/Telegram opt in via `SIBERFLOW_SUBAGENT=true`
